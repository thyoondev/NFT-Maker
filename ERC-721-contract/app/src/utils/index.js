...
const saveMintedItems = (tokenId) => {
  const previousMinted = localStorage.getItem('CYBERCAT.minted');

  if (previousMinted === null) {
    localStorage.setItem('CYBERCAT.minted', tokenId);
  } else {
    const mintedArray = previousMinted.toString().split(',');
    if (mintedArray.indexOf(tokenId.toString()) < 0) {
      localStorage.setItem('CYBERCAT.minted', previousMinted + ',' + tokenId);
    }
  }
...
export { getMetadata, getTokenURI, saveMintedItems, isMintedItems };
